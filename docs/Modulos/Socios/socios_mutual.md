---
sidebar_position: 0
sidebar_label: Socios mutual
---

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

# 👪 Socios

:::tip Rol
El módulo **Socios** es el corazón operativo de **VIMARX**.  
Desde aquí se estructura, administra y vincula toda la información de los asociados, funcionando como base troncal que conecta **datos personales, comerciales, documentales y operativos**.
:::

---

## 🖥️ 1. Vista Principal del módulo Socios

![Barra de tareas vista socios](/img/modulos/socio/sociosBarra.png)
![Vista de socios](/img/modulos/socio/sociosVista_blur.png)

<Tabs>
<TabItem value="Principales" label="⚙️ Funciones Principales">

- ➕ Generar un nuevo socio  
- 🧬 Clonar un socio  
- 🗑️ Eliminar el registro de un socio  
- ✍️ Registrar firma  
- 💳 Crear cuenta AMV  
- 🏷️ Asignar / Cambiar categoría  
- ⛔ Dar de baja  
- ✉️ Enviar email  
- 📌 Registrar operaciones extraordinarias  
- 🔍 Filtrar campos  
- 📤 Exportar socio INAES  
- 📥 Importar socios  

</TabItem>

<TabItem value="Secundarias" label="🛠️ Funciones Secundarias">

- 📸 Capturar imagen de la vista  
- 👤 Asignar cobrador  
- 🪪 Crear credencial  
- 🚨 Generar alertas UIF  
- 🔄 Reestablecer configuración  
- 📄 Mostrar registros DDJJ PEP  
- 📝 Generar resumen  
- 🔎 Consultar CUIT / UIF  
- 🌐 Consultar central  

</TabItem>
</Tabs>

---

## 📋 2. Ficha de un Socio

![Barra de tareas de ficha de un socio](/img/modulos/socio/socioBarra.png)
![Vista de socio](/img/modulos/socio/socioVista_blur.png)


<Tabs>
<TabItem value="Principales" label="⚙️ Funciones Principales">

- ➕ Generar nuevo socio  
- 🗑️ Eliminar socio  
- 👤 Asignar cobrador  
- 💳 Crear cuenta AMV  
- 🏷️ Asignar / Cambiar categoría  
- ⛔ Dar de baja  
- 🚨 Generar alertas UIF  
- 📌 Registrar operaciones extraordinarias  
- ✅ Aprobar / Rechazar perfil  
- 🧾 Verificar AFIP  
- 📊 Calcular riesgo  
- 📥 Importar socios  

</TabItem>

<TabItem value="Secundarias" label="🛠️ Funciones Secundarias">

- 📸 Capturar imagen de la vista  
- ✍️ Registrar firma  
- 🪪 Crear credencial  
- ✉️ Enviar email  
- 🔐 Mostrar permisos por usuario  
- 📝 Generar resumen  
- 📋 Generar tareas  
- 🔎 Consultar CUIT / UIF  

</TabItem>
</Tabs>
